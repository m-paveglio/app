...
cnpjPrestador: string | null = null;
consultaData = {
  numeroNfse: '',
  serieNfse: '',
...
nfses: any[] = [];
selectedNfse: any = null;
displayDialogDetalhes = false;
statusOptions = [
  ...
  consultarNfse() {
    if (!this.cnpjPrestador) {
      this.messageService.add({
        severity: 'error',
        summary: 'Erro',
        detail: 'CNPJ do prestador não identificado'
      });
      return;
    }
    ...
    this.loading = true;
    const params = {
      cnpjPrestador: this.cnpjPrestador,
      ...this.consultaData,
      status: this.consultaData.status === 'TODOS' ? null : this.consultaData.status
    };

    this.nfseService.consultarNfse(this.cnpjPrestador!).subscribe({
      next: (response) => {
        this.nfses = response || [];
        this.messageService.add({
          severity: 'success',
          summary: 'Sucesso',
          detail: `Consulta realizada com ${this.nfses.length} resultados`
        });
        this.loading = false;
      },
      error: (error) => {
        this.messageService.add({
          severity: 'error',
          summary: 'Erro',
          detail: 'Falha ao consultar NFSe: ' + (error.error?.message || error.message)
        });
        this.loading = false;
      }
    });
  ...
  visualizarDetalhes(nfse: any) {
    this.selectedNfse = nfse;
    this.displayDialogDetalhes = true;
  }

  cancelarNfse(nfse: any) {
    this.messageService.add({
      severity: 'warn',
      summary: 'Confirmação',
      detail: `Deseja realmente cancelar a NFSe ${nfse.numero}?`,
      sticky: true,
      key: 'confirmCancel',
      data: nfse,
      life: 10000
  ...
  onConfirmCancel(event: any) {
    const nfse = event.message.data;
    
    this.nfseService.cancelarNfse(nfse.id).subscribe({
      next: () => {
        this.messageService.clear('confirmCancel');
        this.messageService.add({
          severity: 'success',
          summary: 'Sucesso',
          detail: `NFSe ${nfse.numero} cancelada com sucesso`
        });
        this.consultarNfse();
      },
      error: (error) => {
        this.messageService.add({
          severity: 'error',
          summary: 'Erro',
          detail: 'Falha ao cancelar NFSe: ' + (error.error?.message || error.message)
        });
      }
    });
  }
  ...
  exportarParaExcel() {
    this.nfseService.exportarParaExcel(this.nfses).subscribe({
      next: (data) => {
        const blob = new Blob([data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `NFSe_${new Date().toISOString().split('T')[0]}.xlsx`;
        a.click();
      ...
      error: (error) => {
        this.messageService.add({
          severity: 'error',
          summary: 'Erro',
          detail: 'Falha ao exportar para Excel: ' + (error.error?.message || error.message)
        });
      }
    });
  }
